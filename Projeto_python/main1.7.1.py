...
def organizaOrdenar(lista):
    dic = dict(lista)

    # Criando uma cópia profunda do dicionário
    lista_copia = dict(copy.deepcopy(dic))

    # Criando um novo dicionário para armazenar os dados com as novas chaves
    novo_dic = {}

    # Criando um contador auxiliar
    aux = 1

    # Iterando sobre os itens da cópia
    for chave, dados in lista_copia.items():
        # Obtendo o nome da conta
        nome_conta = aux

        # Adicionando uma nova entrada ao novo dicionário com a chave sendo o nome da conta
        novo_dic[nome_conta] = dados

        # Incrementando o contador auxiliar
        aux += 1

    return novo_dic

# ordena por data os recebimentos e pagamentos


def ordenar(dic):
    # transformando o 'dic' em um dado do tipo 'list - lista' para usar o método 'sorted'
    dic = list(dic.items())
    # Ordenar o dicionário com base na data
    dic_ordenado = sorted(dic, key=lambda x: (
        x[1][2][2], x[1][2][1], x[1][2][0]))

    # transfromando de volta em um dado do tipo 'dict - dicionário' para ficar no formato compatível com o restante do código
    dic = dict(dic)
    dic_ordenado = organizaOrdenar(dic_ordenado)
    # Reconstruir o dicionário ordenado
    dic.clear()
    dic.update(dic_ordenado)
...
def listar(dic, tipo):
    if not dic:
        ...
        aux = dict(copy.deepcopy(dic))
        aux.clear()
        aux.update(ordenar(dic))
        for indice, dados in aux.items():
